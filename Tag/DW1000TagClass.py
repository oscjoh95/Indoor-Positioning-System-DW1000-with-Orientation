...
class DW1000Tag():
    def __init__(self, moduleName, ss, irq, antennaDelay, uniqueID, dataLen):
        ...
        self._operationMode = [None] * 6 # [dataRate, pulseFrequency, pacSize, preambleLength, channel, preacode]
        self.callbacks = {}
    ...
    def softReset(self):
        ...
        """
        self.pmscctrl0 = [0] * 4
        self.readBytes(C.PMSC, C.PMSC_CTRL0_SUB, self.pmscctrl0, 4)
        self.pmscctrl0[0] = C.SOFT_RESET_SYSCLKS
        self.writeBytes(C.PMSC, C.PMSC_CTRL0_SUB, self.pmscctrl0, 4)
    ...
    def manageLDE(self):
        ...
        """
        self.pmscctrl0 = [None] * 4
        self.otpctrl = [None] * 2
        self.readBytes(C.PMSC, C.PMSC_CTRL0_SUB, self.pmscctrl0, 4)
        self.readBytes(C.OTP_IF, C.OTP_CTRL_SUB, self.otpctrl, 2)
        ...
        self.writeBytes(C.PMSC, C.PMSC_CTRL0_SUB, self.pmscctrl0, 2)
        self.writeBytes(C.OTP_IF, C.OTP_CTRL_SUB, self.otpctrl, 2)
    ...
    def enableMode(self, mode):
        ...
        # setDataRate
        self.rate = mode[0]
        self.rate = self.rate & C.MASK_LS_2BITS
        self._txfctrl[1] = self._txfctrl[1] & C.ENABLE_MODE_MASK1
        self._txfctrl[1] = self._txfctrl[1] | ((self.rate << 5) & C.MASK_LS_BYTE)
        if self.rate == C.TRX_RATE_110KBPS:
            self.setBit(self._syscfg, 4, C.RXM110K_BIT, True)
        else:
            self.setBit(self._syscfg, 4, C.RXM110K_BIT, False)

        if self.rate == C.TRX_RATE_6800KBPS:
        ...
        if self.rate == C.TRX_RATE_850KBPS:
            self.sfdLength = [C.SFD_LENGTH_850KBPS]
        elif self.rate == C.TRX_RATE_6800KBPS:
            self.sfdLength = [C.SFD_LENGTH_6800KBPS]
        else:
            self.sfdLength = [C.SFD_LENGTH_OTHER]
        self.writeBytes(C.USR_SFD, C.SFD_LENGTH_SUB, self.sfdLength, 1)
        self._operationMode[C.DATA_RATE_BIT] = self.rate

        # setPulseFreq
        self.freq = mode[1]
        self.freq = self.freq & C.MASK_LS_2BITS
        self._txfctrl[2] = self._txfctrl[2] & C.ENABLE_MODE_MASK2
        self._txfctrl[2] = self._txfctrl[2] | (self.freq & C.MASK_LS_BYTE)
        self._chanctrl[2] = self._chanctrl[2] & C.ENABLE_MODE_MASK3
        self._chanctrl[2] = self._chanctrl[2] | ((self.freq << 2) & C.MASK_LS_BYTE)
        self._operationMode[C.PULSE_FREQUENCY_BIT] = self.freq

        # setPreambleLength
        self.prealen = mode[2]
        self.prealen = self.prealen & C.MASK_NIBBLE
        self._txfctrl[2] = self._txfctrl[2] & C.ENABLE_MODE_MASK4
        self._txfctrl[2] = self._txfctrl[2] | ((self.prealen << 2) & C.MASK_LS_BYTE)
        if (self.prealen == C.TX_PREAMBLE_LEN_64 or self.prealen == C.TX_PREAMBLE_LEN_128):
            self._operationMode[C.PAC_SIZE_BIT] = C.PAC_SIZE_8
        elif (self.prealen == C.TX_PREAMBLE_LEN_256 or self.prealen == C.TX_PREAMBLE_LEN_512):
            self._operationMode[C.PAC_SIZE_BIT] = C.PAC_SIZE_16
        elif (self.prealen == C.TX_PREAMBLE_LEN_1024):
    ...
    def generalConfiguration(self, address, mode):
        ...
        currentShortAddress[1] = currentAddress[1] * randint(0, 256)
    ...
    def getFirstPathPower(self):
        ...
        self.readBytes(C.RX_FQUAL, C.PP_AMPL3_SUB, fpAmpl3Bytes, 2)
    ...
    def getReceivePower(self):
       ...
       Returns:
        ...
        cirPwrBytes = [None] * 2
        rxFrameInfo = [None] * 4
        self.readBytes(C.RX_FQUAL, C.CIR_PWR_SUB, cirPwrBytes, 2)
        self.readBytes(C.RX_FINFO, C.NO_SUB, rxFrameInfo, 4)
    ...
    def getReceiveQuality(self):
        ...
        noiseBytes = [None] * 2
        fpAmpl2Bytes = [None] * 2
        self.readBytes(C.RX_FQUAL, C.STD_NOISE_SUB, noiseBytes, 2)
        self.readBytes(C.RX_FQUAL, C.FP_AMPL2_SUB, fpAmpl2Bytes, 2)
    ...
    def getReceiveTimestamp(self):
        ...
        """
        rxTimeBytes = [0] * 5
        self.readBytes(C.RX_TIME, C.RX_STAMP_SUB, rxTimeBytes, 5)
        timestamp = 0
        for i in range(0, 5):
    ...
    def getTransmitTimestamp(self):
        ...
        """
        txTimeBytes = [0] * 5
        self.readBytes(C.TX_TIME, C.TX_STAMP_SUB, txTimeBytes, 5)
        timeStamp = 0
        for i in range(0, 5):
    ...
    def writeBytes(self, cmd, offset, data, dataSize):
        ...
        Args:
                ...
                offset: The offset for the register.
                data: The array containing writeBytes the data you want written.
